...
export default function Sidebar({ width = 256 }: { width?: number }) {
    return (
        <aside style={{ width }} className="flex-shrink-0 bg-surface-light border-r border-border-light h-full flex flex-col shadow-[2px_0_8px_rgba(0,0,0,0.02)] overflow-hidden">
            {/* Logo */}
            <div className="h-[72px] px-6 flex items-center gap-3 border-b border-border-light sticky top-0 bg-surface-light z-10">
                ...
                <h1 className="text-xl font-bold bg-gradient-to-r from-stone-800 to-stone-500 bg-clip-text text-transparent">ServiceGraph</h1>
            </div>

            {/* Nav Items */}
            <nav className="flex-1 overflow-y-auto pt-6 px-4 space-y-6 custom-scrollbar pb-8">
                <div>
                    <h2 className="text-[11px] font-bold text-text-secondary uppercase tracking-widest px-3 mb-3">拓扑总览</h2>
                ...
                <div>
                    <h2 className="text-[11px] font-bold text-text-secondary uppercase tracking-widest px-3 mb-3">链路分析</h2>
                    <ul className="space-y-1">
                        <li>
                ...
                <div>
                    <h2 className="text-[11px] font-bold text-text-secondary uppercase tracking-widest px-3 mb-3">服务管理</h2>
                    <ul className="space-y-1">
                        <li>
                            <NavLink to="/services" className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${isActive ? 'bg-primary text-white shadow-md shadow-primary/20' : 'text-stone-600 hover:bg-stone-100 hover:text-stone-900'}`}>
