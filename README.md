...
- **组员A**：负责底层数据结构（定长最小堆、无向图、解决冲突的哈希表）架构研发；核心联通图算法（含带路径溯源的一度/二度人脉推扫、最短社交距离BFS）设计；系统主 GUI 引擎与前后端事件循环生命周期联调。
- **组员B**：负责 Top-K 个性化好友智能推荐策略（特征计算与堆排）实现；社交网络可视化交互图谱（基于 Networkx）渲染呈现；各子级面板事件绑定及系统边界异常拦截处理。
- **组员C**：负责内存数据与本地持久化文件（CSV/TXT）双向读写引擎构建；完成节点编辑（防孤立点检查与关系联动删除）闭环；研发拼音模糊搜索组件及集成式测试数据的校验与装载清洗。
