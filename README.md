...
- [x] **2026-02-18** `feat/init`: 自研底层结构及BFS核心图计算算法封装完毕
- [x] **2026-02-19** `feat`: 完成基础功能包括直接查找、二度查找、计算距离
- [x] **2026-02-20** `feat`: 添加智能推荐
- [x] **2026-02-21** `feat`: 添加网络图谱
- [x] **2026-02-21** `fix`: 修复网络图谱每次都不一样的bug
- [x] **2026-02-21** `feat`: 添加设置与全局字号放缩功能

## 接下来准备开发

为进一步提升系统的商用价值及吞吐量，后续迭代计划如下：
- [ ] 支持用户在可视化图谱上动态增删节点与连线（可视化编辑引擎）
- [ ] 接入专属图数据库（如 Neo4j）以支持百万级海量节点的亚秒级查询
- [ ] 增加更多维度的图特征算法分析（如页面排名 PageRank、社区聚类发现）
- [ ] 引入持久层关系型数据库（如 SQLite / MySQL）替代当前的 CSV/TXT 离线静态文件流读写
