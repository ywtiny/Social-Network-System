...
- [x] **2026-02-16** `feat/init`: 完成图基础结构和BFS算法编写，搭建底层图核心数据结构与 BFS 算法框架。
- [x] **2026-02-17** `feat`: 完成控制台基础功能与控制面板，实现一度/二度人脉查找及社交距离测算。
- [x] **2026-02-18** `feat`: 引入智能推荐功能，初步完成核心逻辑闭环；实装基于 `networkx` 的可视化网络图谱。
- [x] **2026-02-19** `ui`: 优化前端排版，将距离测算等原先粗糙的交互改为下拉框联动；新增顶层菜单栏和缩放控制。
...
- [x] **2026-02-22** `refactor`: 大幅重构全平台人员搜索框（主控台、修改信息、添加好友），自主研发 `FriendPanel`，实现全域无缝拼音联想过滤，并修复输入法抢占焦点的一系列恶性 Bug。系统最终完全体定稿。
